...
int main() 
{
	...
	cout << "You can use sqrt(x) and pow(x,y) functions too." << endl;
...
double hatvany(){
   Token t =ts.get();
    if(t.kind!='('){
        error("Incorrect input! Use this function like this: 'pow(*expression*,integer)");
    }
    double d = expression();
...
double negyzetgyok(){
    ...
    switch (t.kind){
        case '(': {
            ...
            {
                error("The expression under a square root can only be 0 or higher.");
            }
        }
        default: {
            ts.putback(t);
            error("Please use the squareroot like this: sqrt(*expression*)");
        }
    }
    return 0;
...
double primary()
{
	...
	switch (t.kind)
	{
        ...
        case sqrt_type:{
            double d = negyzetgyok();
            t = ts.get();
            if (t.kind != ')') error("Please use the squareroot like this: sqrt(*expression*)");
	        return d;
        }
        case pow_type:{
            double d = hatvany();
            t = ts.get();
            if (t.kind != ')') error("Please use the power like this: pow(*expression*, integer)");
            return d;
        }
		case number:
			return t.value;
